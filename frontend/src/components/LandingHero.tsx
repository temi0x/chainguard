...
const LandingHero: React.FC<LandingHeroProps> = ({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden py-16 px-4">
      ...
      />
      <div
        className={`absolute bottom-1/3 right-1/3 w-40 h-40 rounded-full animate-float ${
          theme === "dark" ? "bg-secondary/5" : "bg-secondary/3"
        }`}
        style={{ animationDelay: "2s" }}
      />
      <div
        className={`absolute top-2/3 left-1/3 w-32 h-32 rounded-full animate-float ${
      ...
      <div className="container mx-auto text-center relative z-10">
        ...
        <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10">
          ChainGuard AI uses advanced machine learning to analyze security,
          financial health, governance, and community sentiment to provide
          comprehensive risk assessments for DeFi protocols.
        </p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {["Coinbase", "Chainlink", "Aave", "Uniswap", "MakerDAO"].map(
              (name, index) => (
                <div
                  key={index}
                  className={`flex items-center justify-center h-12 px-6 rounded-lg border border-border ${
